...
When analyzing the validation results of the Mask R-CNN model, trained on a fusion of RGB, SAR, and Rao's Q data, attention should be paid to an important interpretative aspect regarding the system's sensitivity.

Despite its high performance in identifying anthropogenic structures, the model's inference process exhibits certain semantic limitations:

The model demonstrates very high accuracy in locating actual archaeological sites. By integrating SAR intensity and landscape heterogeneity (Rao's Q), the network reliably captures subtle morphological and vegetation anomalies that are invisible in traditional aerial photography.

It was noted that the model tends to classify any significant background signal interference as potential sites. In practice, this means that any strong spectral or radar anomaly—resulting from, for example, geological processes, contemporary agricultural practices, or landform microforms—may be misinterpreted as an archaeological feature (False Positive).

This is because the SAR radiometric signature and the Rao's Q biodiversity index respond to physical variations in the substrate, which are crucial in archaeology but not unique to it. Operating at a time scale of 250 epochs and a loss of 0.5, the model optimized the detection of differences but still requires additional expert validation to distinguish between settlement traces and natural environmental processes.

Therefore, the results generated by the model should be considered a probability map of anthropogenic anomalies. Each prediction requires secondary filtering using contextual analysis or field verification to separate actual cultural heritage from environmental noise.

### The model is continually being refined.
